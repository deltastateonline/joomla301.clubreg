 ...
 *
 */
class ClubregModelPaymentReporting extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$all_string['payments'] = "pt.`payment_id`, pt.`payment_key`, 
		a.`member_id`,pt.`payment_transact_no`, pt.`payment_date`, pt.`payment_season`,
		pm.`config_name` as `payment_method`,
		ps.`config_name` as `payment_status`,
		pd.`config_name` as `payment_desc`,
		pt.`payment_notes`, pt.`payment_amount`";
		$all_string[] = "date_format(a.created, '%d/%m/%Y %H:%i:%s') as created";
		...
		$query->join('LEFT', $db->quoteName(CLUB_PAYMENTS_TABLE).' AS pt ON pt.member_id = a.member_id');
		$query->join('LEFT', '#__users AS usert ON pt.created_by = usert.id');
		$query->join('LEFT', $db->quoteName(CLUB_CONFIG_TABLE).' AS pd ON pt.payment_desc = pd.config_short');
		$query->join('LEFT', $db->quoteName(CLUB_CONFIG_TABLE).' AS pm ON pt.payment_method = pm.config_short');
		$query->join('LEFT', $db->quoteName(CLUB_CONFIG_TABLE).' AS ps ON pt.payment_status = ps.config_short');
