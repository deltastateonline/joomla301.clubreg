...
class dropShadow
{
	...
	var $final_width;	// final width
	var $final_height;		// final height
	var $_force_check ; //determine if to use file exisit 
	...
	function dropShadow($debug = FALSE)
	{
		...
		$this->_shadows = array();
		$this->final_width = $this->final_height = 0;
		$this->_force_check = false;
	}
	...
	function _resizeImage($nx, $ny, $function)
	{
		...
		$this->_imgFinal = @ImageCreateTrueColor($nx, $ny); // create the new image 
		@ImageCopyResampled($this->_imgFinal, $this->_imgOrig, 0, 0, 0, 0, $nx, $ny, @ImageSX($this->_imgOrig), @ImageSY($this->_imgOrig));	
		
		if(intval($this->final_width) > 0 and intval($this->final_height) > 0){
			$another_img = @ImageCreateTrueColor($this->final_width, $this->final_height); // create the new image 
			$white = @imagecolorallocate($another_img, 255, 255, 255); // create white back ground
			imagefilledrectangle($another_img, 0, 0, $this->final_width-1,$this->final_height-1, $white);	// create a white rectangle
			
			$f_x =  @ImageSX($this->_imgFinal); // get the width of the final image
			$f_y = @ImageSY($this->_imgFinal); // get the height of the final image
			$l_x = 0.5 * ($this->final_width - $f_x); // get the central location x
			$l_y = 0.5 * ($this->final_height - $f_y); // get the central location y
			
			@ImageCopy($another_img, $this->_imgFinal, $l_x, $l_y, 0, 0,  $f_x, $f_y); //copy the image into the new			
			
			$this->_imgFinal = $another_img; // reassign			
	...
	function _saveImage($filename, $img, $type, $quality)
	{
		...
		if ($ext == 'gif') $saved = @$func($img, $filename);
		if ($saved == FALSE)
		{
	...
	{
		if ($this->_showDebug){
			$fp = fopen("image_debug.txt","a+");				
			$all_str =  "<p><strong style=\"color:#FF0000\">".date("d/m/Y H:i:s")."Debug message from function $function:</strong> $string</p>\n";
			fwrite($fp,$all_str);	
			fclose($fp);
